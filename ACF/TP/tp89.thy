...
fun delete::"transaction \<Rightarrow> transaction list \<Rightarrow> transaction list"
where
"delete _ [] = []"
|"delete s (x#xs) = (if (x=s) then xs else (x#(delete s xs)) )"


(* 2] Dans la liste de transactions validées, tout triplet (c,m,id) n'apparait qu'une seule fois *)
lemma lem2:" ((l1 = export (traiterMessageList l)) \<and> (l2 = (delete x l1))) \<longrightarrow> (delete x l1 = delete x l2) "
quickcheck
oops

(*  3] Une transaction peut etre annulée meme après validation *)
lemme lem3:"(l = traiterMessageList l1) \<and> (List.member (export l) (t,n)) \<longrightarrow> (traiterMessage (Cancel l) l = l2 \<and> \<not>(List.member (export l1) (t,n))) "
quickcheck
oops
