...
class Bank(validator : TransValidator) extends Subject{
	...
	def extMessage2message(m:ExtMessage):message=
	  m match{
	  	case ExtPay(c,m,tid,am) => if (c>=0 && m>=0 && tid>= 0) Pay((Nat.Nata(c),(Nat.Nata(m),Nat.Nata(tid))),Nat.Nata(am)) else throw new IllegalArgumentException("Negative numbers for client, merchant or transaction")
	  	case ExtAck(c,m,tid,am) => if (c>=0 && m>=0 && tid>= 0) Ack((Nat.Nata(c),(Nat.Nata(m),Nat.Nata(tid))),Nat.Nata(am)) else throw new IllegalArgumentException("Negative numbers for client, merchant or transaction")
